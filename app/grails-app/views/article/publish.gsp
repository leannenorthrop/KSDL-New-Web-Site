...
<html>
  <head>
    ...
    <title><g:message code="article.publish.title" args="${[fieldValue(bean:publishableInstance,field:'title')]}"/></title>
  <g:javascript>
    $(function() {
    $("#publish").validate();
    });
...
<g:form name="publish" action="publish" method="post">
  ...
  <shiro:hasRole name="Author">
    <fieldset>
      ...
      <g:select name="category" from="${['M','N','C','W','B','A','H','S','R']}" value="${publishableInstance?.category}" valueMessagePrefix="publish.category" class="required ui-corner-all ${hasErrors(bean:publishableInstance,field:'category','errors')}"/>
      </p>
      <p>
    ...
    <fieldset>
      ...
      <g:render template="/contentWithPreview" model="[previewController: 'article',publishableInstance:publishableInstance]"/>
    </fieldset>
  </shiro:hasRole>
  <shiro:lacksRole name="Author">
    <g:render template="/article" model="[publishableInstance: publishableInstance, articles:[]]"/>
  </shiro:lacksRole>
  <fieldset>
